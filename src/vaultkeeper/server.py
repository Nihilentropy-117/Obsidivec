...
# LLM call log — DEBUG level so it doesn't clutter the main console output
_llm_log_path = "/data/llm_calls.log"
_llm_file_handler = logging.FileHandler(_llm_log_path)
_llm_file_handler.setLevel(logging.DEBUG)
_llm_file_handler.setFormatter(
    logging.Formatter("%(asctime)s %(message)s")
)
_llm_logger = logging.getLogger("vaultkeeper.llm.client")
...
async def app_lifespan(server: FastMCP) -> AsyncIterator[AppContext]:
    ...
    logger.info(f"ChromaDB path: {config.index.db_path}")
